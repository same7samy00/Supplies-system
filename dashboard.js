...
function initializeApp(user) {
    ...
    connectScannerBtn.addEventListener('click', () => {
        ...
        const scannerUrl = `${window.location.protocol}//${window.location.host}/scanner.html?session=${currentScanSessionId}`;
    ...
    document.getElementById('generateBarcodeBtn').addEventListener('click', () => {
        ...
        const randomPart = Math.random().toString().substring(2, 8); // e.g., 123456
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (targetViewId === 'inventoryView') {
                searchInput.focus();
            }
        });
    });

    // --- All other app logic (CRUD, Search, Print) goes here...
}
